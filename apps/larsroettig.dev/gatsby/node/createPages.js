...
module.exports = async ({ actions: { createPage }, graphql }, themeOptions) => {
  ...
  try {
    ...
    authors = localAuthors.data.authors.edges.map(normalize.local.authors);

    articles = localArticles.data.articles.edges.map(normalize.local.articles);
